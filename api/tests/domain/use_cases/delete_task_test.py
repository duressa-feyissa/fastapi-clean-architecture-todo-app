...
from app.domain.use_cases.delete_task import DeleteTask, Params
from app.domain.repositories.base_repository import BaseRepository
from app.domain.entities.task import TaskEntity, Task
from core.common.either import Either
from core.errors.failure import Failure

class TestDeleteTask(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.use_case = DeleteTask(self.repository)
        
    async def test_delete_task_success(self):
        mock_task = TaskEntity.from_dict(self.tTask)
        params = Params(task_id=mock_task.id)
        
        self.repository.delete_task.return_value = Either.right(mock_task)
        
        result = await self.use_case(params)
        self.assertTrue(result.is_right())
        self.assertEqual(result.get(), mock_task)
        self.repository.delete_task.assert_called_once_with(mock_task.id)
        
    async def test_delete_task_failure(self):
        mock_task = TaskEntity.from_dict(self.tTask)
        params = Params(task_id=mock_task.id)
        
        self.repository.delete_task.return_value = Either.left(Failure())
